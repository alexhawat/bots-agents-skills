---
name: discogs-bot
description: Automate Discogs (collection, wantlist, marketplace, orders, cart) by replaying a signed-in session via scripts referenced from a clone of alexhawat/bots-agents-skills.
---

# Discogs-Bot (OpenClaw skill)

Automates Discogs from a signed-in session. The runnable scripts are **not** in
this skill — they live in a clone of the `alexhawat/bots-agents-skills`
repository. You have shell access; run the scripts in place from the clone.
...
`_lib/`. Run from the pack dir in the clone:
