...
# _lib/envfile.py -> parents[2] is grok_bots/
SIBLING = (
    Path(envfile.__file__).resolve().parents[2]
    / "discogs-bot" / "discogs-scripts" / "_lib" / "envfile.py"
)
...
def test_cookie_value_containing_equals_survives(tmp_path):
    """Env values are full of '='. Splitting on the first one only is load-bearing."""
    p = tmp_path / "a.env"
    p.write_text("COOKIE=sid=xyz==; session=abc=\n")
    assert parse_env(p)["COOKIE"] == "sid=xyz==; session=abc="
...
def test_envfile_matches_discogs_pack_copy():
    ...
    if not SIBLING.is_file():
        pytest.skip("sibling discogs-bot pack not present (box install)")
